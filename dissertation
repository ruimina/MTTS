...
韵律结构预测是中文语音合成中非常重要的一部分，关系到合成出语音的自然度和流畅度，本毕业设计将在实现中文语音合成系统的基础上着重实现、分析并比较不同韵律结构预测方法，列出不同方法的优劣以及适用范围并进一步探究神经网络在韵律结构预测上的效果。

毕业设计思路：

本毕业设计将参考大量相关的论文，在基本实现中文语音合成系统的基础上进一步研究韵律结构预测。具体工作包括了：

\* 搭建或购买基础的语音合成语料库和韵律结构标注数据库
...
李明老师实验室的相关研究成果（如语音识别）以及语料库资源。

进度安排：

2017年底前

\* 通过开源系统merlin深入理解并实现英文语音合成；

\* 实现前端文本分析包括分词、词性标注、多音字识别、拼音标注等基础功能；
...
\* 对韵律结构预测方法展开深入研究，研究个性化语音和情感语音合成在韵律结构层面上的特征。

## **中文统计参数语音合成系统的实现与改进**

# 摘要

随着语音识别技术的成熟，人们逐渐把研究焦点转向了语音合成系统。_虽然语音合成已有多年的研究历史，但到目前为止，网络上尚未有基于统计参数语音合成的开源中文语音合成系统，也没有开源的中文语音合成语料库，而英文的语音合成所需的基础语料库和系统早已开源。_随着语音合成音质的改善，人们对语音合成系统也提出了更高的要求，如要求个性化（特定人）的语音合成，情感语音合成，与此同时，人们也希望语音合成系统有更高的灵活性和实时性，能够利用有限的语音资源合成优质的语音。统计参数的语音合成系统因其对韵律的灵活控制，不需要很大规模的语料库成为了人们的选择，基于此，本文研究了基于统计参数的语音合成系统，同时综合利用最新的语音合成技术，在英文语音合成系统merlin的基础上搭建了一个开源的、具有灵活性、易于整合和修改的中文语音合成系统，以便今后的研究更好地开展。
...
6.2.1 合成歌声 见https://www.zhihu.com/question/26165668
...
### **3.1.1 King\_tts\_003语料库**

语料库使用购买自海天瑞声公司的King\_tts\_003语料库，该语料库由专业的标准女播音员录制，总时长为15小时，近2万个句子。其中标注包括了拼音、韵律（韵律边界与重音）、音素发音时长、声韵母标注。语音文件以44.1 KHz，16bit，双音道，windows的无压缩PCM格式存储。除了此外，该语料库还提供了记录EGG（electroglottography）信号的音频。
...
l d表示句中的静音长度小于100ms的停顿。

l s表示句子的起始点和结束点以及句中大于100ms的停顿。
...
Based on unit definition and contextual factors, we define five categories for the questions in

the question set. The five categories of the question set are sub-syllable, syllable, phrase, song,

and note. The details of the question set are described as follows.
...
category of the initial, and pronunciation category of the final

(2) Syllable: The number of sub-syllables in a syllable and the position of the syllable in the

note

(3) Phrase: The number of sub-syllables/syllables in a phrase

(4) Song: Average number of sub-syllables/syllables in each measure of the song and the

number of phrases in this song

(5) Note: The absolute/relative pitch of the note; the key, beat, and tempo of the note; the

length of the note by syllable/0.1 second/thirty-second note; the position of the current

note in the current measure by syllable/0.1 second/ thirty-second note; and the position of

the current note in the current phrase syllable/0.1 second/thirty-second note
